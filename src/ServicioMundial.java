...
public class ServicioMundial {
    ...
    /**
     * Reemplaza el mundial activo. Permite crear uno nuevo desde el menú.
     */
    ...
    /**
     * Devuelve todas las sedes registradas en el mundial actual.
     */
    public List<Sede> getSedes() {
        if (mundial == null) return new ArrayList<>();
        return mundial.getSedes();
    }

    /**
     * Devuelve todos los estadios de todas las sedes del mundial.
     */
    public List<Estadio> getEstadios() {
        List<Estadio> todos = new ArrayList<>();
        if (mundial == null) return todos;
        for (Sede s : mundial.getSedes()) {
            todos.addAll(s.getEstadios());
        }
        return todos;
    }

    /**
     * Devuelve todos los árbitros registrados en todos los países.
     */
    public List<Arbitro> getArbitros() {
        List<Arbitro> todos = new ArrayList<>();
        for (Pais p : paises) {
            if (p.getArbitros() != null) {
                todos.addAll(p.getArbitros());
            }
        }
        return todos;
    }
    ...
    public void registrarPartido(Partido partido) throws ArbitrajeInvalidoException {
        ...
        // Validación del equipo de árbitros
        partido.validarArbitrajeCompleto();
        ...
        for (Seleccion sel : selecciones) {
            if (sel.getJugadores() != null) {
                for (Jugador j : sel.getJugadores()) {
                    if (j.calcularGoles() > 0 && !goleadores.contains(j)) {
                        goleadores.add(j);
                    }
                }
            }
    ...
    public String generarFichaTecnicaPartido(Partido partido) {
        ...
        String infoEstadio = (partido.getEstadio() != null)
            ? partido.getEstadio().getNombre() + " (" + partido.getEstadio().getSede().getCiudad() + ")"
            : "Sin estadio asignado";
